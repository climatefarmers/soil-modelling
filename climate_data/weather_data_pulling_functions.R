...
GoogleDriveDesktop_SharedFiles_path <- "H:/Shared Drives"

setwd(+'Climate Farmers/07_Tech/Modelling/soil-modelling/climate_data/sis-biodiversity-cmip5-regional_data/')

# location of interest
lon_farmer <- -8.211738  # longitude of location
lat_farmer <- 38.583577 # latitude  of location
...
get_monthly_mean_temperature <- function(lon_farmer,lat_farmer){
  ...
  ncpath <- "sis-biodiversity-cmip5-regional-rainfall-temperature/"
  ncname <- "downscaled_monthly_rainfall_temperature_1x1km"#"downscaled_monthly_rainfall_temperature_1x1km"
  ncfname <- paste(ncpath, ncname, ".nc", sep="")
  # open a netCDF file
  obsdata <- nc_open(ncfname)
  # get dates
...
get_monthly_mean_precipitation <- function(lon_farmer,lat_farmer){
  ...
  # set path and name of netCDF file
  ncpath <- "sis-biodiversity-cmip5-regional-rainfall-temperature/"
  ncname <- "downscaled_monthly_rainfall_temperature_1x1km"#"downscaled_monthly_rainfall_precipitation_1x1km"
  ncfname <- paste(ncpath, ncname, ".nc", sep="")
  # open a netCDF file
  obsdata <- nc_open(ncfname)
  # get dates
...
get_monthly_mean_pevap <- function(lon_farmer,lat_farmer){
  ...
  # set ncpath and 
  ncpath <- "sis-biodiversity-cmip5-regional-potential_evapotranspiration/"
  ncname <- "downscaled_monthly_pevaporation_1x1km"
  ncfname <- paste(ncpath, ncname, ".nc", sep="")
  # open a netCDF file
  obsdata <- nc_open(ncfname)
  # get dates
