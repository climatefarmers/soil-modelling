...
parameters <- fromJSON(file.path(working_dir, "parameter_files", "init_validation.json"))
...
source(file.path(weatherDB_loc, "weather_data_pulling_functions.R"))
...
cinput_montado_bl<-parcel_Cinputs$tot_Cinputs[parcel_Cinputs$parcel_ID=="NC" & parcel_Cinputs$scenario=="current"]

mean_input$field_carbon_in <- cinput_montado_bl #0.5*(Cinput_leading_to_observed_SOC_past_land_use+cinput_montado_bl)
time_horizon = 30
