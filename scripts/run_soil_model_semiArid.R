...
run_soil_model <- function(soil_loc,project_loc,project_name,modelling_data_loc,weatherDB_loc){
  ...
  mean=c(list(rep(0,12)),
         ...
         list(c(1,rep(NA,11))), # mean potential transpiration to open-pan evaporation convertion rate
  ...
  for(i in 1:length(Cinputs)){
    ...
    model1 <- calc_carbon_over_time(time_horizon,
                                    ...
                                    precip = mean_input$past_precip,#*850/sum(mean_input$past_precip),
                                    evap = mean_input$past_evap,#*945/sum(mean_input$past_evap),
                                    soil_thick = mean_input$soil_thick[1],
                                    clay = 30, #mean_input$clay[1],
                                    pE = mean_input$pE[1],
  ...
  for (n in c(1:n_run)){
    ...
    for(i in c(1:nrow(parcel_inputs))){
      ...
      C0_df <- calc_carbon_over_time(time_horizon,
                                     ...
                                     version="Semi-arid",
                                     silt = 30,
                                     bulk_density = 1.3) # Attention: is there any tillage in the scenarios?
